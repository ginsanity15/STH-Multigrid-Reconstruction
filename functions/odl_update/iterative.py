...
def conjugate_gradient_normal(op, x, rhs, niter=1, callback=None, **kwargs):
...
Conjugate_gradient_on_the_normal_equations>`_.
    ...
    Keywords Argument
    ----------
    time_slot : stores the time CG spends on each iteration, which is more accurate
    on illustrating if calculation time has been saved
    
    diff : stores the dffierence between op(x) and rhs, which can be used to determine
    when will the output reach the optimal outcome
    ...
    for _ in range(niter):
        ...
        if time_slot is not None:
            time_slot.append(time.time() - start_time)
            
        if diff is not None:
            diff.append(np.sum(np.abs(op(x) - rhs)))
            
    if time_slot is not None and diff is None:
        return time_slot
        
    if time_slot is None and diff is not None:
        return diff
        
    if time_slot is not None and diff is not None:
        return time_slot, diff
