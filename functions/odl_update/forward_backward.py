...
def forward_backward_pd(x, f, g, L, h, tau, sigma, niter,
    ...
    op = kwargs.pop('RayTrafo', None)
    data = kwargs.pop('Sinogram', None)
    if op is not None and data is not None:
        op = op
        data = data
        curve = []
        
    time_slot = kwargs.pop('timeslot', None)
    ...
    for k in range(niter):
        ...
        if callback is not None:
            callback(x)
        if op is not None and data is not None:
            curve.append(np.sum(np.abs(data-op(x))))
        if time_slot is not None:
           time_slot.append(time.time() - start_time)
        if minimization is not None:
            minimization.append(np.abs(g[0](L[0](x))+h(x)))
    
    if op is not None and data is not None and time_slot is None:
        return curve
    if time_slot is not None and data is None:
        return time_slot
    if op is not None and data is not None and time_slot is not None and minimization is not None:
        return curve, time_slot, minimization
