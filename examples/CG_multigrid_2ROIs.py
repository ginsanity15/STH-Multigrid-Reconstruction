...
# -*- coding: utf-8 -*-
"""
Created on Sat Jun 24 15:52:50 2017
...
filename_f1 = output_store_path + 'CG_coarse_space_' + str(coarse_length) + '_fine1'
filename_f2 = output_store_path + 'CG_coarse_space_' + str(coarse_length) + '_fine2'
...
insert_min_pt2 = [-5, 0, -5]
insert_max_pt2 = [5, 10, 5]
...
index1 = np.floor((np.array(insert_min_pt2) - np.array(min_pt))/coarse_discr.cell_sides)
insert_min_pt2 = np.array(min_pt) + coarse_discr.cell_sides*index1
index2 = np.floor((np.array(max_pt) - np.array(insert_max_pt2))/coarse_discr.cell_sides)
insert_max_pt2 = np.array(max_pt) - coarse_discr.cell_sides*index2
...
coarse_mask2 = multigrid.operators.MaskingOperator(coarse_discr, insert_min_pt2, insert_max_pt2)
...
insert_discr2 = odl.uniform_discr_fromdiscr(
                fine_discr, min_pt=insert_min_pt2, max_pt=insert_max_pt2,
                cell_sides=fine_discr.cell_sides)
...
insert_ray_trafo2 = odl.tomo.RayTransform(insert_discr2, geometry,impl='astra_cuda')
    
# Forward operator = sum of masked coarse ray trafo and insert ray trafo
sum_ray_trafo = odl.ReductionOperator(masked_coarse_ray_trafo, insert_ray_trafo1, insert_ray_trafo2)
...
# %% Storing the image
# TODO: Check if it is possible to store it in DICOM file (this could ease a lot
# of pain for displaying)
store_as_dcm(reco, filename_c, filename_f1, filename_f2)
...
f = open(filename_f1,'wb')
roi1 = reco[1].asarray()
pickle.dump(roi1,f)
f = open(filename_f2,'wb')
roi2 = reco[2].asarray()
pickle.dump(roi2,f)
